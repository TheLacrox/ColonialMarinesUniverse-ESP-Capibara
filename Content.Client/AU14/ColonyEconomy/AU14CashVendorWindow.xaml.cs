...
public sealed partial class AU14CashVendorWindow : FancyWindow
{
    ...
    public void UpdateState(AU14CashVendorBuiState state)
    {
        ...
        TaxFooterLabel.Text = state.SalesTaxPercent > 0
            ? Target("cmu-ambassador-ui-sales-tax", $"Sales Tax: {state.SalesTaxPercent:F0}%", ("percent", state.SalesTaxPercent.ToString("F0")))
        ...
        ScanIdBtn.Text = state.HasDepartmentMode
            ? Target("cmu-colony-economy-clear-department", "Clear Dept")
            : Target("cmu-colony-economy-scan-id", "Scan ID");
    ...
    private void RefreshList()
    {
        ...
        foreach (var item in _lastState.Items)
        {
            ...
            var buyBtn = new Button { Text = Target("cmu-colony-economy-buy", "Buy"), MinWidth = 50 };
        ...
        if (ItemList.ChildCount == 0)
            ItemList.AddChild(new Label { Text = Target("cmu-colony-economy-no-items-available", "No items available.") });
    }
}
