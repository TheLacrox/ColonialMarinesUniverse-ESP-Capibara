...
public sealed partial class GuidebookWindow : FancyWindow, ILinkClickHandler, IAnchorClickHandler
{
    ...
    private void ShowGuide(GuideEntry entry)
    {
        ...
        if (!_parsingMan.TryAddMarkup(EntryContainer, entry))
        {
    ...
    {
        var fallback = _loc.TryGetString(entry.Name, out var name)
            ? name
            : entry.Name;
        return CMUPrototypeLocalization.GetGuideEntryName(_loc, entry.Id, fallback);
    }
